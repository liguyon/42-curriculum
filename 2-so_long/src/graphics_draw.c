...
/*   Updated: 2023/10/27 14:57:22 by liguyon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	graphics_draw_line(t_graphics *grph, t_pixel p0, t_pixel p1)
{
    ...
    dda = (t_dda){.delta_x = p1.x - p0.x, .delta_y = p1.y - p0.y};
    if (dda.delta_x == 0) {
...
void	graphics_draw_circle(t_graphics *grph, t_circle c, t_color color)
{
	...
	{
		graphics_draw_pixel(grph,
		c.x + c.radius * cosf(theta * M_PI / 180.0f),
		c.y + c.radius * sinf(theta * M_PI / 180.0f),
		color);
	}
}
